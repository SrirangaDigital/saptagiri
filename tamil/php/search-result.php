...
if ($num_results > 0)
{
    echo '<h1 class="clr1 gapBelowSmall">முடிவுகள்- ' . toKannada(intval($num_results)) . '</h1>';
}
...
if($num_rows > 0)
{
    while($row = $result->fetch_assoc())
    {
		...
		if($row['maasa'] != '')
		{
			$info = $info . ', ' . $row['maasa'] . '&nbsp;மாதம்';
		}
		if($row['samvatsara'] != '')
		{
			$info = $info . ', ' . $row['samvatsara'] . '&nbsp;மாலை';
		}
        ...
        if ((strcmp($id, $row['titleid'])) != 0) {
			...
			echo '		<span class="aIssue clr5"><a href="toc.php?vol=' . $row['volume'] . '&amp;part=' . $row['part'] . '">தொகுதி ' . toKannada(intval($row['volume'])) . ', வெளியீடு ' . toKannada($dpart) . ' <span class="font_resize">(' . $info . ')</span></a></span>';
            echo '  </div>';
            ...
            //~ }
            echo '<br/><span class="downloadspan"><a href="downloadPdf.php?titleid='.$titleid.'" target="_blank">Download Pdf</a> </span>';
            $id = $row['titleid'];
        }
        else 
        {
