...
if($yearMonth['month'] != '')
{
	$info = $info . getTeluguMonth($yearMonth['month']);
}
if($yearMonth['year'] != '')
{
	$info = $info . ' <span class="font_size">' . toTelugu(intval($yearMonth['year'])) . '</span>';
}
if($maasa['maasa'] != '')
{
	$info = $info . ', ' . $maasa['maasa'] . '&nbsp;మాసము';
}
if($maasa['samvatsara'] != '')
{
	$info = $info . ', ' . $maasa['samvatsara'] . '&nbsp;సంవత్సరము';
}
$info = preg_replace("/^,/", "", $info);
$info = preg_replace("/^ /", "", $info);

echo '<h1 class="clr1 gapBelowSmall">సంపుట ' . toTelugu(intval($volume)) . ', సంచికే ' . toTelugu($dpart) . ' <span style="font-size: 0.85em">(' . $info . ')</span></h1>';
